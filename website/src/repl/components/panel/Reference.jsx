...
export function Reference() {
  ...
  return (
    <div className="flex h-full w-full p-2 overflow-hidden text-[var(--fs-body)] text-foreground">
      <div className="h-full  flex flex-col gap-2 w-1/3 max-w-72">
        <div class="w-full flex">
          <Textbox className="w-full text-[var(--fs-input)]" placeholder="Search" value={search} onChange={setSearch} />
        </div>
        <div
          className="flex flex-col h-full overflow-y-auto  gap-1.5 rounded-md"
          style={{ backgroundColor: 'color-mix(in srgb, var(--background) 30%, transparent)' }}
        >
          {visibleFunctions.map((entry, i) => (
            <a
              key={i}
              className="cursor-pointer text-foreground flex-none hover:bg-lineHighlight overflow-x-hidden px-1 text-[var(--fs-input)] text-ellipsis"
              onClick={() => {
      ...
      >
        <div className="prose dark:prose-invert min-w-full px-1 text-foreground ref-content">
          <h2>API Reference</h2>
          <p>
          ...
          {visibleFunctions.map((entry, i) => (
            <section key={i}>
              ...
              {entry.examples?.map((example, j) => (
                <pre
                  key={j}
                  className="text-foreground text-[var(--fs-input)]"
                  style={{ backgroundColor: 'color-mix(in srgb, var(--background) 30%, transparent)' }}
                >
                  {example}
                </pre>
