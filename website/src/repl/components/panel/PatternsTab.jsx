...
function UserPatterns({ context }) {
  ...
  const viewingPatternID = viewingPatternData?.id;
  return (
    <div className="flex flex-col gap-1.5 flex-grow overflow-hidden h-full pb-1">
      <div className="pr-2 space-x-3 flex max-w-full overflow-x-auto scrollbar-hide">
        <ActionButton
          label="new"
          onClick={() => {
      ...
      <div
        className="overflow-auto h-full p-1.5"
...
function PatternPageWithPagination({ patterns, patternOnClick, context, paginationOnChange, initialPage }) {
  ...
  return (
    <div className="flex flex-grow flex-col  h-full overflow-hidden justify-between">
      <div
        className="overflow-auto flex flex-col flex-grow p-1.5"
        style={{ backgroundColor: 'color-mix(in srgb, var(--background) 30%, transparent)' }}
      >
        <PatternButtons
...
export function PatternsTab({ context }) {
  ...
  return (
    <div className="px-3 py-2 w-full text-foreground space-y-1.5 flex flex-col overflow-hidden max-h-full h-full">
